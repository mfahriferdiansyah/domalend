...
contract AIOracle is Ownable, AccessControl {
    ...
    function requestScoring(uint256 domainTokenId) external {
        ...
        // Create task in ServiceManager if address is set
        if (serviceManagerAddress != address(0)) {
            IDomalendServiceManager(serviceManagerAddress).createDomainScoringTask(domainTokenId);
        }
    }
    ...
    function setServiceManager(address _serviceManagerAddress)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_serviceManagerAddress != address(0), "Invalid ServiceManager address");
        serviceManagerAddress = _serviceManagerAddress;
        // Grant SCORING_SERVICE_ROLE to ServiceManager so it can submit scores
        _grantRole(SCORING_SERVICE_ROLE, _serviceManagerAddress);
    }
